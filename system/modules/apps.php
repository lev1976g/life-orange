	...
	switch($act){
		...
		case "app":
			...
			if($row['status'] == '1' or $row['user_id'] == $user_id){
				...
				if($row['img'] == '') $img = '/templates/Default/images/no_apps.gif'; else $img = '/uploads/apps/'.$row['id'].'/100_'.$row['img'];
		...
		case"save_settings":
			...
			if($hash == md5($key.'asd4465a4das56d4as65'.$user_id) && $balance >= '0'){
				...
				if($balance <= $row['user_balance']) {
					...
					$db->query("INSERT INTO `".PREFIX."_historytab` SET user_id = '{$user_id}', type = '8', price='{$balance}', status = '-', date = '{$server_time}'");
		...
		case"mywall":
			...
			if($sql['img'] == '') $img = '/templates/Default/images/no_apps.gif'; else $img = '/uploads/apps/'.$sql['id'].'/'.$sql['img'];
		...
		case"search":
			...
			if($row_app['img']){
				...
				$application_img = '/templates/Default/images/no_apps.gif';
		...
		case"doload":
			...
			foreach($sqlls_ as $rowsa){
			    ...
			    $ava = $config['home_url'].'uploads/apps/'.$rowsa['user_id'].'/50_'.$rowsa['user_photo'];
				...
				<div class="apps_application">
					...
					<a class="apps_mr" href="/apps?i='.$rowsa['id'].'" onClick="apps.view(\''.$rowsa['id'].'\', this.href, \'/apps\'); return false">

					<img src="/uploads/apps/'.$rowsa['id'].'/100_'.$rowsa['img'].'" class="fl_r" width="50" height="50" onMouseOver="myhtml.title(\''.$rowsa['id'].'\', \''.$rowsa['title'].'\', \'apps_gane\')" id="apps_gane'.$rowsa['id'].'" /></a>
		...
		foreach($sqls_ as $rowsds){
				...
				if($rowsds['img'] == '') $img = '/templates/Default/images/no_apps.gif'; else $img = '/uploads/apps/'.$rowsds['id'].'/100_'.$rowsds['img'];
		...
		foreach($sqll_ as $rows){
			...
			if($rows['img'] == '')	$img = '/templates/Default/images/no_apps.gif'; else $img = '/uploads/apps/'.$rows['application_id'].'/100_'.$rows['img'];
		...
		foreach($sqlls_ as $rowsa){
			...
			if($rowsa['img'] == '')	$img = '/templates/Default/images/no_apps.gif'; else $img = '/uploads/apps/'.$rowsa['id'].'/100_'.$rowsa['img'];
		...
		foreach($sql_ as $rowsd){
			...
			if($rowsd['cols'] >= 2){
				...
				if($rowsd['img'] == '')	$img = '/templates/Default/images/no_apps.gif'; else $img = '/uploads/apps/'.$rowsd['id'].'/100_'.$rowsd['img'];
