				...
				if($row) {
					...
					$sql_us = $db->super_query("SELECT user_id, user_photo, user_search_pref FROM `".PREFIX."_users` WHERE user_id = '{$row_com['user_id']}'");
							...
							if(date('Y-m-d', $row_com['date']) == date('Y-m-d', $server_time))
								$date = langdate('сегодня в H:i', $row_com['date']);
					...
					$avau = 'templates/Default/images/no_ava_50.png';
					$tm = time()-2;
					...
					$text = replase_smile($text);
						...
						<div id=\''.$new.'\' style=\'float:left;width:740px;background: '.$newcom.';border-bottom: solid 1px #c9c9c9;border-right: solid 1px #c9c9c9;-moz-border-radius: 2px;-webkit-border-radius: 2px;border-radius: 2px;padding: 10px;margin-bottom: 5px;\'>
...
<div class=\'clear\'></div>
					...
					} else { $tpl->set('{comms}',  '<span style=\'color: #a7adb1;font-weight: 700;font-size: 14px;text-shadow: 0 1px 0 rgba(255,255,255,0.3);\'>Оставь первое сообщение</span>');}
		...
		default:
			...
			$tpl->load_template('chat/main.tpl');
				...
				if($row) {
					...
					$text = replase_smile($text);
                        $workcom .= '<div class=\'fotocom\' style=\'margin-bottom:5px;\' id=\'comment_'.$row_com['id'].'\'><div class=\'ava_mini\' style=\'float:left;width:60px\'><a href=\'/u'.$row_com['user_id'].'\' onclick="Page.Go(this.href); return false"><img src=\''.$avau.'\' style=\'border-radius: 5px;\'></a></div>
						<div style=\'float:left;width:740px;background: #fff;border-bottom: solid 1px #c9c9c9;border-right: solid 1px #c9c9c9;-moz-border-radius: 2px;-webkit-border-radius: 2px;border-radius: 2px;padding: 10px;margin-bottom: 5px;\'>
...
<div class=\'clear\'></div>
				...
				if($sql_us['user_photo'])
					...
					$tpl->set('{myava}', 'templates/Default/images/no_ava_50.png');
