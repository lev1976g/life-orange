...
require_once "wt_conf.php";    //файл с классом работы с БД и объявлением констант модуля
require_once "wt_class.php";	//основная часть модуля, обработка .tpl
$wt = new Wt();
...
if ( !isset($wt_webid) or empty($wt_webid) ) {
    ...
    $name		= iconv('utf-8','windows-1251',$_REQUEST['app_name']);
    $message	= iconv('utf-8','windows-1251',$_REQUEST['message']);
    ...
    if ($method == 'getAppFriends'){// получаем друзей пользователя
    ...
    else if ($method == 'getProfiles'){//получаем профили по уидс
        $users = explode(',', $uids);
        $result_content = '<?xml version="1.0" encoding="UTF-8"?><response>';
        ...
        if (!opendir($new_image)){//если нет такого файла, то копируем его
    ...
    echo iconv('windows-1251','utf-8',$result_content);
    exit();

} 
else if (isset($wt_load) and $wt_load=='AJAX') {
    $tmp = preg_split("/_escaped_fragment_=/",$_SERVER['REQUEST_URI']);
    if (isset($tmp[1]) && $tmp[1]) {$tmp1=preg_split("/&/",$tmp[1]);$tmp[1]=$tmp1[0];}//исключаем данные после амперсанда
    ...
    if (isset($escaped_fragment)){
        ...
        if ($wt_version == '2'){//индексация 2ой версии
            if ($request[0] == 'gameid'){
                $wt_game = file_get_contents( WEBTOMAT_DIR .'/themes/gamebrd/wt_game.tpl');
                if ( $config['charset'] == "windows-1251" ) $wt_game = iconv( "UTF-8", "windows-1251", $wt_game );
                $wt_cont = $wt->wt_ingame2($wt_game,$request[1],'#!genre_');
            }
            else{
                $wt_board = file_get_contents( WEBTOMAT_DIR .'/themes/gamebrd/wt_board.tpl');
                if ( $config['charset'] == "windows-1251" ) $wt_board = iconv( "UTF-8", "windows-1251", $wt_board );
        ...
        else {//индексация 1ой версии
            if ($request[0] == 'gameid'){
                $ingame = $request[1];
                $_REQUEST['ingame']=$ingame;
                $wt_cont = file_get_contents( WEBTOMAT_DIR .'/themes/default/wt_ingame.tpl');
                if ( $config['charset'] == "windows-1251" ) $wt_cont = iconv( "UTF-8", "windows-1251", $wt_cont );
                $wt_cont = $wt->wt_ingame($wt_cont,$ingame);
            }
            else {
                if ($request[0] == 'genre' || $request[0] == 'cat' || $request[0] == 'tag'){//жанр
                    $wt_cont = file_get_contents( WEBTOMAT_DIR .'/themes/default/wt_list.tpl');
                    if ( $config['charset'] == "windows-1251" ) $wt_cont = iconv( "UTF-8", "windows-1251", $wt_cont );
                ...
                else {//главная страница каталога
                    if ($request[0] == 'a' && isset($request[1]) && $request[1] == 'tags') {
                        $_REQUEST['a'] = 'tags';
                    }
                    $wt_cont = file_get_contents( WEBTOMAT_DIR .'/themes/default/wt_main.tpl');
                    if ( $config['charset'] == "windows-1251" ) $wt_cont = iconv( "UTF-8", "windows-1251", $wt_cont );
    ...
    if ( isset($wt_ajax) and isset($wtid) ){	//принимаем AJAX с id пользователя и передаем его на запись в сессию

        exit($wt->wt_sessWtId($wtid));

    }
    elseif ( isset($rate) and isset($ingame) ) {	//принимаем AJAX с голосом и id игры, передаем на проверку и запись в БД

        exit($wt->wt_rate($rate,$ingame,$wt_id));

    }
    elseif ( isset( $ingame ) ) {		//обработка страницы с игрой
    ...
    } elseif ( isset( $ingenre ) or isset( $cat ) or isset( $search ) or isset( $p ) or isset( $intag ) ) {		//обработка страниц с выводом спика игр

        $wt_cont = file_get_contents( WEBTOMAT_DIR .'/themes/default/wt_list.tpl');
        if ( $config['charset'] == "windows-1251" ) $wt_cont = iconv( "UTF-8", "windows-1251", $wt_cont );

        if ( isset($wt_ajax) ) {		//если запрос пришел через AJAX, готовим на вывод только список игр.
    ...
    else {	//обработка главной страницы и страницы "все теги"
        $wt_cont = file_get_contents( WEBTOMAT_DIR .'/themes/default/wt_main.tpl');
        if ( $config['charset'] == "windows-1251" ) $wt_cont = iconv( "UTF-8", "windows-1251", $wt_cont );
        if (!$a) $a='main';
        $_REQUEST['a']='main';
    ...
    /* Производим замену переменных в шаблоне */
    if (!$wt_skey) {
        $auth = file_get_contents( WEBTOMAT_DIR.'/themes/default/wt_login.tpl' );
        if ( $config['charset'] == "windows-1251" ) $auth  = iconv( "UTF-8", "windows-1251", $auth  );
...
$auth = '';
if ($wt_skey){/* авторизация через cms */
