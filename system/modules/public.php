...
if($logged){
	...
	if($page_cnt){
		...
		$row = $db->super_query("SELECT id, title, onv, descr, traf, ulist, photo, date, admin, feedback, comments, real_admin, pages, pages_num, rec_num, del, ban, adres, audio_num, forum_num, discussion, status_text, web, videos_num, cover, cover_pos FROM `".PREFIX."_communities` WHERE ".$sql_where."");
