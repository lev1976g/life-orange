...
function wt_msg($msg_type, $msg_title, $msg_text, $msg_link){
  if (function_exists('msg')){
	msg($msg_type, $msg_title, $msg_text, $msg_link );
  } else { 
    msgbox ( $msg_title, $msg_text, $msg_link);
  }
}
...
if( $action == "dosavewtopt" ) {
    ...
    if( empty( $editwebid ) || strlen( $editwebid ) > 10 ) {
	  wt_msg( "Error!", "Error", "webID not correct", "?mod=webtomat");
    }
    ...
    if( empty( $editload ) ) {
      wt_msg( "Error!", "Error", "Load type not correct", "?mod=webtomat");
	}
