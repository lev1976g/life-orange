...
<ol class="breadcrumb">
	...
	<h2 class="page-header">Жалобы</h2>
...
<div class="table-responsive">
	<div class="col-lg-12">
	    ...
	    <table class="table table-bordered table-hover table-striped">
			<tbody>
				<tr>
					<td style="height:30px;"><h6>Поиск по ID пользователя:</h6></td>
					<td class="col-lg-4"><input type="text" name="se_uid" class="form-control form-cascade-control" value="{$se_uid}" /></td>
				</tr>
				<tr>
					<td><h6>Жалоба на:</h6></td>
					<td>  <select name="act" class="form-control form-cascade-control">
...
foreach($sql_ as $row){
	...
	if($row['act'] == 'photo'){
		...
		if(!$row_info['user_id'])
			$data_X = '<div class="alert alert-info">Фотография удалена.</div>';
		else
		...
		if(!$row_info_video['title'])
			$data_X = '<div class="alert alert-info">Видеозапись удалена.</div>';
		else
		...
		if(!$row_info_note['title'])
			$data_X = '<div class="alert alert-info">Заметка удалена.</div>';
		else
		...
		if(!$row_info_rec['text'])
			$data_X = '<div class="alert alert-info">Запись удалена.</div>';
		else
