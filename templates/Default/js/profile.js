...
var groups = {
	...
	saveinfo: function(id){
		...
		if(pattern.test(adres_page)){
			...
			$.post('/index.php?go=groups&act=saveinfo', {id: id, title: title, descr: descr, onv: onv, comments: comments, pages: $('#pages').val(), adres_page: adres_page, discussion: $('#discussion').val(), web: $('#web').val()}, function(d){
				if(d == 'err_adres')
...
var pages = {
	New: function(i){
		if($('#title_n').val() != 0){
			if($('#text').val() != 0 || $('#vaLattach_files').val() != 0){
				butloading('forum_sending', 70, 'disabled');
				$.post('/index.php?go=public_pages&act=new_send', {public_id: i, title: $('#title_n').val(), text: $('#text').val(), attach_files: $('#vaLattach_files').val()}, function(d){
					Page.Go('/pages'+i+'?act=view&id='+d);
				});
			} else
				setErrorInputMsg('text');
		} else
	...
	Page: function(p){
		if($('#load_pages_page_lnk').text() == 'Показать больше страниц'){
			textLoad('load_pages_page_lnk');
			$.post('/index.php?go=public_pages&public_id='+p, {a: '1', page: page}, function(d){
				page++;
				$('#PagesPage').append(d);
				$('#load_pages_page_lnk').text('Показать больше страниц');
				if(!d){
					$('#'+$('.pages_bg2:last').attr('id')).css('margin-bottom', '-15px');
					$('#pages_page_lnk').hide();
					$('#load_pages_page_lnk').text('');
				}
			});
		}
	},
	...
	SaveEdit: function(i){
		$('#editClose').hide();
		butloading('saveedit', 55, 'disabled');
		$.post('/index.php?go=public_pages&act=saveedit', {text: $('#editText').val(), fid: i}, function(d){
			if(!at) at = '';
			$('#teckText').html(d+'<span id="attach">'+at+'</span>');
			pages.CloseEdit();
			butloading('saveedit', 55, 'enabled', 'Сохранить');
	...
	SaveEditTitle: function(f){
		if($('#title').val() != 0){
			pages.CloseEditTitle();
			$('#editTitleSaved').text($('#title').val());
			$.post('/index.php?go=public_pages&act=savetitle', {fid: f, title: $('#title').val()});
		} else
			setErrorInputMsg('title');
	},
	...
	Fix: function(f){
		settings.privacyClose('msg');
		if($('#fix_text').text() == 'Сделать главной'){
			$('#fix_text').text('Не делать главной');
			$('.pages_infos_div').html('<b>Страница главная.</b><br />Теперь эта страница всегда будет выводиться над остальными в списке страниц.').fadeIn('fast');
		} else {
			$('#fix_text').text('Сделать главной');
			$('.pages_infos_div').html('<b>Страница больше не главная.</b><br />Эта страница будет выводиться на своем месте в списке страниц.').fadeIn('fast');
		}
		$.post('/index.php?go=public_pages&act=fix', {fid: f});
	},
	DelBox: function(f, p){
		settings.privacyClose('msg');
		Box.Show('del_forthe', 350, lang_title_del_photo, '<div style="padding:15px;" id="del_status_text_pages">Вы уверены, что хотите удалить эту страницу?</div>', lang_box_canсel, lang_box_yes, 'pages.StartDelete('+f+', '+p+')');
	},
	StartDelete: function(f, p){
		$('#box_loading').show();
		ge('box_butt_create').disabled = true;
		$('#del_status_text_forum').text('Тема удаляется..');
		$.post('/index.php?go=public_pages&act=del', {fid: f}, function(d){
			Page.Go('/pages'+p);
		});
	},
		...
		if(title != 0){
			if(text != 0){
				Box.Page('/index.php?go=public_pages&act=preview', 'text='+text+'&title='+title, 'preview', 820, lang_notes_preview, lang_msg_close, 0, 0, 500, 1, 1, 1, 0, 0);
			} else {
				$("#text").html('');
				$("#text").css('background', '#ffefef').focus();
				setTimeout("$('#text').css('background', '#fff').focus()", 800);
